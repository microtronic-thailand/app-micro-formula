...
**Phase:** 2.5 - Buyer Edition & Local Engine Setup
**Status:** ✅ แยกโปรเจกต์เวอร์ชันลูกค้า & รันระบบผ่าน Docker สำเร็จ (Ready for Delivery)
...
### 5. Database Schema (Supabase & Local Docker)
- **Defined SQL Schema**: `supabase/schema.sql` (ตาราง Customers, Invoices, Invoice Items)
- **Local Engine**: เตรียม `docker-compose.yml` สำหรับรัน PostgreSQL และ pgAdmin ในตัว
- **Sync Strategy**: วางแผนระบบ Offline-first (IndexedDB) และ Background Sync เพื่อความเสถียร
- **Data Service**: สร้าง `lib/data-service.ts` และ `lib/postgres.ts` รองรับทั้ง Cloud และ Local DB

### 6. Buyer Edition Transformation (White Label)
- **Project Split**: แยกโปรเจกต์ออกเป็น 4 Repositories หลัก (App, Engines, Manual, Dev-Docs)
- **Branding**: เปลี่ยนหน้า Landing Page เป็นหน้าจอล็อกอินพรีเมียมสำหรับลูกค้าโดยเฉพาะ
- **Clean UI**: ลบส่วนประกอบการตลาด (Pricing/Ads) ออกจากตัวโปรเจกต์
- **Deployment**: Push ข้อมูลทั้งหมดขึ้น GitHub: `microtronic-thailand/app-micro-formula`
